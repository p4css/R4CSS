...
-   附註：在幾乎所有程式語言中，單等號`=`指的是assignment，把右方的算式、值或物件指給左方的變數。而比較兩者相不相等，則用雙等號`==`，例如`1==3-2`。R當初在設計時可能是為了避免混淆，所以把assignment的符號設計為`<-`。
...
如果對每個運算中的數字都創建一個對應的變數，工作量將會無限膨脹且難以管理。因此，我們通常會利用其他序列或資料結構（如 `vector`、`list` 或 `data.frame`）來一次性儲存多筆資料。

在 R 語言中，`vector` 是一種基本資料類型，由相同資料型別的元素組成。它可以儲存數值、文字、邏輯值等不同類型的資料。例如，下列程式碼創建了一個包含整數的 `vector`：`my_vector <- c(1, 2, 3, 4, 5)`。要存取 vector 中的第一個元素，只需使用索引方式 `my_vector[1]`，這表示取得 `vector` 第一個位置的內容（依此類推，第二個、第三個元素以此類推）。

此外，在撰寫程式碼時，所有文字必須以成對的雙引號或單引號括起來，以區分變數名稱與數字常數。也就是說，未被引號包圍的英文字母通常代表變數或函式名稱，而若數字被引號包住，則會被視為文字型資料。
