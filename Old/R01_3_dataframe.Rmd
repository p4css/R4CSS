...
myfetchData <- function(dbname){
  ...
  # for mac
  # res <- dbSendQuery(conn, "SELECT urls.url, visits.visit_time, visits.visit_duration, visits.id, visits.from_visit, visits.transition, visits.segment_id, urls.id FROM visits INNER JOIN urls ON visits.url=urls.id;")
  ...
  res <- dbSendQuery(conn, "SELECT urls.url, 
                     ...
                     visits.id, visits.from_visit, visits.transition, visits.segment_id, urls.id 
