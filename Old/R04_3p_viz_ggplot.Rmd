---
title: "Visualization Introduction: Using ggplot2"
author: "Jilung Hsieh"
date: "`r Sys.Date()`"
...
本節著重在介紹`ggplot`的基本概念與設定，如果想要了解各類圖表及其適用情形，可參考後續章節。
...
**小秘訣：**可在一開始便透過`knitr::opts_chunk$set(echo = TRUE, fig.width = 2, fig.asp = 0.4)`來一次設定所有圖片。`fig.width = 8`與`fig.height = 6` 是以英吋（inches）為單位，或用`fig.dim = c(8, 6)`一次設定長寬[^1]。`echo = TRUE`是設定knit出輸出格式（如html）時，也要包含程式碼。如果`echo = FALSE`的話，就只會輸出文字和圖形。
...
1.  **指定要進行繪圖`ggplot()`**：用`%>%`將資料（dataframe）pipe給`ggplot()`後，\
    底下各增添的繪圖選項都用**`+`**的符號，類似不斷修正繪圖結果的意思。
2.  **`aes()`**：指定X／Y軸與Grouping factor。指定圖表的X/Y軸分別是什麼變數，有些圖表只需要單一個變數（例如Density-chart和Histogram），有些需要X/Y兩個變數（例如Scatter-chart）什麼的變數要做視覺化，Boxplot甚至可以直接指定最大、最小、Q1、Q3和Median等多個變數。
...
NW %>%    
    ...
    geom_point(size=2, color="deeppink", alpha=0.5, shape=18) + 
    theme_minimal()
```
